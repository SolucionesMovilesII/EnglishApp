...
import { ChapterRepetition, RepetitionStatus, SessionType } from '../../domain/entities/chapter-repetition.entity';
...
export class ChapterRepetitionRepository implements IChapterRepetitionRepository {
  ...
  async findByUserId(userId: string, filters?: RepetitionFilters): Promise<ChapterRepetition[]> {
    const queryBuilder = this.repository.createQueryBuilder('repetition')
    ...
    if (filters?.sessionType) {
      queryBuilder.andWhere('repetition.sessionType = :sessionType', { sessionType: filters.sessionType });
    }
  ...
  async findByUserAndChapter(userId: string, chapterId: string, filters?: RepetitionFilters): Promise<ChapterRepetition[]> {
    const queryBuilder = this.repository.createQueryBuilder('repetition')
      .leftJoinAndSelect('repetition.originalProgress', 'originalProgress')
      .where('repetition.userId = :userId', { userId })
  ...
  async markAsCompleted(id: string, score: number, exerciseResults?: Record<string, any>): Promise<ChapterRepetition> {
  ...
  async getRepetitionStats(userId: string, chapterId?: string): Promise<RepetitionStats> {
    ...
    const averageScore = completedCount > 0 
      ? completedRepetitions.reduce((sum, r) => sum + (r.repetitionScore || 0), 0) / completedCount
      : 0;
    
    const lastRepetitionDate = repetitions.length > 0 
    ...
    const improvementRates = completedRepetitions
      .filter(r => r.originalProgress && r.originalProgress.score)
      .map(r => r.getImprovementRate(r.originalProgress.score));
  ...
  async countRepetitionsByChapter(userId: string): Promise<Record<string, number>> {
    const result = await this.repository.createQueryBuilder('repetition')
