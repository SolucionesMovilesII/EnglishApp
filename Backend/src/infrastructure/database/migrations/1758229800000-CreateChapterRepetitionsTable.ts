...
export class CreateChapterRepetitionsTable1758229800000 implements MigrationInterface {
  ...
  public async up(queryRunner: QueryRunner): Promise<void> {
    ...
    // Habilitar RLS
    await queryRunner.query(
      `ALTER TABLE "chapter_repetitions" ENABLE ROW LEVEL SECURITY`
    );
    ...
    await queryRunner.query(
      `GRANT SELECT ON "chapter_repetitions" TO anon`
    );
    
    await queryRunner.query(
      `GRANT ALL PRIVILEGES ON "chapter_repetitions" TO authenticated`
    );
  }
  ...
  public async down(queryRunner: QueryRunner): Promise<void> {
    ...
    // Eliminar índices
    await queryRunner.query(
      `DROP INDEX IF EXISTS "IDX_chapter_repetitions_created_at"`
    );
    
    await queryRunner.query(
      `DROP INDEX IF EXISTS "IDX_chapter_repetitions_status"`
    );
    
    await queryRunner.query(
      `DROP INDEX IF EXISTS "IDX_chapter_repetitions_original_progress"`
    );
    
    await queryRunner.query(
      `DROP INDEX IF EXISTS "IDX_chapter_repetitions_chapter_id"`
    );
    
    await queryRunner.query(
      `DROP INDEX IF EXISTS "IDX_chapter_repetitions_user_id"`
    );
