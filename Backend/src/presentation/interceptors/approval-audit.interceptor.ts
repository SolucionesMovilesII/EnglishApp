...
export class ApprovalAuditInterceptor implements NestInterceptor {
  ...
  intercept(context: ExecutionContext, next: CallHandler): Observable<any> {
    ...
    this.logger.log(
      `[${requestId}] APPROVAL_OPERATION_START: ${method} ${url}`,
      {
        requestId,
        method,
        url,
  ...
  private logOperationSpecifics(
    ...
    if (url.includes('/evaluate') && method === 'POST') {
      this.logger.log(
        `[${requestId}] APPROVAL_EVALUATION_ATTEMPT`,
        {
          requestId,
    ...
    if (url.includes('/batch-evaluate') && method === 'POST') {
      this.logger.log(
        `[${requestId}] APPROVAL_BATCH_EVALUATION_ATTEMPT`,
        {
          requestId,
          evaluatorUserId: user?.userId,
          evaluationCount: body?.evaluations?.length || 0,
          targetUserIds: body?.evaluations?.map((e: any) => e.userId) || [],
          chapterIds: body?.evaluations?.map((e: any) => e.chapterId) || [],
          timestamp: new Date().toISOString(),
        },
      );
    }
    ...
    if (url.includes('/rules') && method === 'POST') {
      this.logger.log(
        `[${requestId}] APPROVAL_RULE_CONFIGURATION`,
        {
          requestId,
    ...
    if (url.includes('/can-attempt') && method === 'GET') {
      this.logger.log(
        `[${requestId}] APPROVAL_ATTEMPT_CHECK`,
        {
          requestId,
  ...
  private logOperationResult(
    ...
    if (url.includes('/evaluate') && method === 'POST') {
      this.logger.log(
        `[${requestId}] APPROVAL_EVALUATION_RESULT`,
        {
          requestId,
          evaluatorUserId: user?.userId,
    ...
    if (url.includes('/batch-evaluate') && method === 'POST') {
      this.logger.log(
        `[${requestId}] APPROVAL_BATCH_EVALUATION_RESULT`,
        {
          requestId,
          evaluatorUserId: user?.userId,
          successfulEvaluations: response?.results?.length || 0,
          failedEvaluations: response?.errors?.length || 0,
          totalProcessed: (response?.results?.length || 0) + (response?.errors?.length || 0),
          duration,
          timestamp: new Date().toISOString(),
        },
      );
    ...
    if (url.includes('/rules') && (method === 'POST' || method === 'PUT')) {
      this.logger.log(
        `[${requestId}] APPROVAL_RULE_CONFIGURATION_RESULT`,
        {
          requestId,
          configuratorUserId: user?.userId,
          ruleId: response?.id,
