...
export class ProgressController {
  ...
  @ApiResponse({
    status: 400,
    description: 'Invalid input data or chapter not approved',
  })
  @ApiResponse({
    status: 401,
    description: 'Unauthorized - Invalid or missing JWT token',
  ...
  @ApiResponse({
    status: 409,
    description: 'Active repetition already exists for this chapter',
  })
  async startRepetition(
    @Body() createRepetitionDto: CreateRepetitionDto,
    @Request() req: AuthenticatedRequest,
  ): Promise<RepetitionResponseDto> {
    this.logger.log(`Starting repetition for user: ${req.user.userId}, chapter: ${createRepetitionDto.chapterId}`);
  ...
  @ApiResponse({
    status: 409,
    description: 'Cannot update completed or abandoned repetition',
  })
  ...
  @ApiResponse({
    status: 400,
    description: 'Invalid score or repetition cannot be completed',
  })
  @ApiResponse({
    status: 401,
    description: 'Unauthorized - Invalid or missing JWT token',
  ...
  @ApiResponse({
    status: 409,
    description: 'Repetition is not active',
  })
  async completeRepetition(
    ...
    this.logger.log(`Completing repetition: ${repetitionId} for user: ${req.user.userId} with score: ${body.score}`);
  ...
  @HttpCode(HttpStatus.OK)
  @ApiOperation({
    summary: 'Abandon repetition',
    description: 'Marks a repetition as abandoned',
  })
  @ApiParam({
    name: 'id',
    description: 'Repetition ID',
  ...
  async abandonRepetition(
    ...
    try {
      return await this.updateRepetitionUseCase.abandonRepetition(
        req.user.userId,
        repetitionId,
      );
    } catch (error) {
