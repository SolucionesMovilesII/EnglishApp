...
describe('ProgressController', () => {
  ...
  describe('startRepetition', () => {
    ...
    it('should handle BadRequestException', async () => {
      // Arrange
      repeatChapterUseCase.execute.mockRejectedValue(new BadRequestException('Chapter not approved'));

      // Act & Assert
      await expect(controller.startRepetition(createRepetitionDto, mockRequest))
  ...
  describe('getRepetitionById', () => {
    it('should return repetition by id', async () => {
      ...
      expect(result).toBeDefined();
      expect(result).toEqual(mockRepetitionResponse);
      expect(getRepetitionsUseCase.getRepetitionById).toHaveBeenCalledWith('user-1', 'repetition-1');
    });

    it('should handle NotFoundException', async () => {
      // Arrange
      getRepetitionsUseCase.getRepetitionById.mockRejectedValue(new NotFoundException('Repetition not found'));

      // Act & Assert
      await expect(controller.getRepetitionById('repetition-1', mockRequest))
  ...
  describe('updateRepetition', () => {
    ...
    it('should update repetition successfully', async () => {
      ...
      expect(updateRepetitionUseCase.execute).toHaveBeenCalledWith('user-1', 'repetition-1', updateDto);
    });
  });
  ...
  describe('completeRepetition', () => {
    ...
    it('should complete repetition successfully', async () => {
      ...
      expect(updateRepetitionUseCase.completeRepetition).toHaveBeenCalledWith('user-1', 'repetition-1', 92, completeBody.exerciseResults);
    });
  });
  ...
  describe('abandonRepetition', () => {
    it('should abandon repetition successfully', async () => {
      ...
      expect(updateRepetitionUseCase.abandonRepetition).toHaveBeenCalledWith('user-1', 'repetition-1');
    });
  });
});