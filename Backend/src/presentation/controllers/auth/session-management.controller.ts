...
export class SessionManagementController extends BaseAuthController {
  ...
  @ApiOperation({
    ...
    description: 'Returns a list of all active sessions for the current user with device and location information.'
  })
  @ApiQuery({ 
  ...
  async getActiveSessions(
    ...
    try {
      ...
      this.logger.error(`Get active sessions failed: ${error instanceof Error ? error.message : String(error)}`, error instanceof Error ? error.stack : undefined);
  ...
  @ApiOperation({
    ...
    description: 'Revokes a specific session by its family ID. The user can only revoke their own sessions.'
  })
  @ApiParam({ 
    name: 'familyId', 
  ...
  async revokeSession(
    ...
    try {
      ...
      const currentFamilyId = (request as any).user?.familyId;
      if (familyId === currentFamilyId) {
        this.logger.warn(`User ${userId} attempting to revoke current session: ${familyId}`);
        throw new ForbiddenException('Cannot revoke your current session. Use logout endpoint instead.');
      }
      ...
      if (!sessionExists) {
        this.logger.warn(`User ${userId} attempted to revoke non-existent or unauthorized session: ${familyId}`);
