...
function createMockUser(overrides: Partial<User> = {}): User {
  return {
    ...
    createdAt: new Date(),
    updatedAt: new Date(),
    get isGoogleAuth() { return false; },
    get isAppleAuth() { return false; },
    get isEmailPasswordAuth() { return true; },
    get isAdmin() { return false; },
    get isSuperAdmin() { return false; },
    ...overrides,
  } as User;
}
...
function createMockApprovalEvaluation(overrides: Partial<ApprovalEvaluation> = {}): ApprovalEvaluation {
  ...
  describe('execute', () => {
    it('should successfully evaluate approval (approved)', async () => {
      ...
      approvalEvaluationRepository.create.mockResolvedValue(
        createMockApprovalEvaluation({ score: 85, threshold: 80, status: EvaluationStatus.APPROVED }),
      );
    ...
    it('should handle rejection with error carryover', async () => {
      ...
      approvalEvaluationRepository.create.mockResolvedValue(
        createMockApprovalEvaluation({ score: 75, threshold: 80, status: EvaluationStatus.REJECTED }),
      );
    ...
    it('should enforce 100% requirement for chapters 4 and 5', async () => {
      ...
      approvalEvaluationRepository.findPreviousAttempts.mockResolvedValue([]);
      approvalEvaluationRepository.create.mockResolvedValue(
        createMockApprovalEvaluation({ chapterId: '4', score: 95, threshold: 100, status: EvaluationStatus.REJECTED }),
      );
    ...
    it('should approve with extraData present', async () => {
      const dto: EvaluateApprovalDto = {
        ...
        chapterId: '1',
        score: 85,
        additionalData: { timeSpent: 450, hintsUsed: 2, questionsAnswered: 10, difficulty: 'medium' },
      };

      const mockRule = createMockApprovalRule({ chapterId: '1', minScoreThreshold: 80, allowErrorCarryover: true });
