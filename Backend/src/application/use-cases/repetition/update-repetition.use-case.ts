...
import { ChapterRepetition, RepetitionStatus } from '../../../domain/entities/chapter-repetition.entity';
...
export class UpdateRepetitionUseCase {
  ...
  async execute(
    ...
    this.logger.log(
      `Updating repetition ${repetitionId} for user: ${userId}`,
    );
  ...
  async abandonRepetition(
    ...
    this.logger.log(
      `Abandoning repetition ${repetitionId} for user: ${userId}`,
    );
    ...
    try {
      ...
      const abandonedRepetition = await this.chapterRepetitionRepository.markAsAbandoned(
        repetitionId,
      );
