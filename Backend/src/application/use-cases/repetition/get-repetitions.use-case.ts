...
import { IChapterRepetitionRepository, RepetitionFilters } from '../../interfaces/repositories/chapter-repetition-repository.interface';
...
export class GetRepetitionsUseCase {
  ...
  async execute(
    ...
    try {
      ...
      if (options.includeStats) {
        ...
        const repetitionsByChapter = await this.chapterRepetitionRepository.countRepetitionsByChapter(userId);
      ...
      const allRepetitions = await this.chapterRepetitionRepository.findByUserId(userId, totalFilters);
  ...
  async getRecentRepetitions(userId: string, limit: number = 10): Promise<RepetitionResponseDto[]> {
    ...
    try {
      ...
      const repetitions = await this.chapterRepetitionRepository.findRecentRepetitions(userId, limit);
  ...
  private mapToResponseDto(repetition: ChapterRepetition): RepetitionResponseDto {
    return {
      ...
      improvementRate: repetition.originalProgress?.score ? repetition.getImprovementRate(repetition.originalProgress.score) : null,
