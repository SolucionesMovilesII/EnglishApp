...
describe('RepeatChapterUseCase', () => {
  ...
  describe('execute', () => {
    ...
    it('should create a new repetition successfully', async () => {
      ...
      expect(chapterRepetitionRepository.findActiveRepetition).toHaveBeenCalledWith('user-1', 'chapter-1');
    ...
    it('should throw NotFoundException when user does not exist', async () => {
      ...
      // Act & Assert
      await expect(useCase.execute('user-1', createRepetitionDto))
    ...
    it('should throw BadRequestException when there is an active repetition', async () => {
      ...
      await expect(useCase.execute('user-1', createRepetitionDto))
        .rejects.toThrow(BadRequestException);
      expect(chapterRepetitionRepository.findActiveRepetition)
        .toHaveBeenCalledWith('user-1', 'chapter-1');
    });
    ...
    it('should throw BadRequestException when chapter IDs do not match', async () => {
      ...
      // Act & Assert
      await expect(useCase.execute('user-1', mismatchedDto))
        .rejects.toThrow(BadRequestException);
    });
  });
});