...
export interface IReadingPracticeRepository {
  ...
  findByUserIdAndCategory(userId: string, category: string, limit?: number, offset?: number): Promise<ReadingPractice[]>;
  findByUserIdAndDifficulty(userId: string, difficulty: string, limit?: number, offset?: number): Promise<ReadingPractice[]>;
  findCompletedByUserId(userId: string, completed: boolean, limit?: number, offset?: number): Promise<ReadingPractice[]>;
